document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof preciosServicios !== 'undefined') {
          ...
          <div class="card">
            <div>
              <div class="card-img-wrapper">
                <img src="${item.imagen || 'assets/salon1.jpeg'}" alt="${item.nombre}">
              </div>
              <h3>${item.nombre}</h3>
    ...
    if (giftcardsContainer) {
      // Create options for all individual massages
      const individualCards = preciosServicios.valores_masajes.map((item) => `
        <div class="card" style="padding: var(--space-md);">
          <div>
            <span class="label-caps">Regalo Masaje</span>
            <h3 style="margin-top: var(--space-sm);">${item.nombre}</h3>
            <p>${item.descripcion}</p>
            <div class="treatment-price-row">
              <h4>Sesión (${item.duracion})</h4>
          ...
          <div style="margin-top: var(--space-md);">
            <a href="https://wa.me/56940415858?text=Hola,%20quiero%20comprar%20una%20Gift%20Card%20de%20${encodeURIComponent(item.nombre)}%20(${item.duracion})" target="_blank" class="btn btn-primary" style="width: 100%;">Comprar Regalo</a>
          </div>
        </div>
      `);

      // Create options for all mixed massages
      const mixtosCards = preciosServicios.masajes_mixtos.map((item) => `
        <div class="card" style="padding: var(--space-md);">
          <div>
            <span class="label-caps">Regalo Mixto</span>
            <h3 style="margin-top: var(--space-sm);">${item.nombre}</h3>
            <p>${item.descripcion}</p>
            <div class="treatment-price-row">
              <h4>Sesión (${item.duracion})</h4>
      ...
      // Create customizable free value option
      const customCard = `
        <div class="card" style="padding: var(--space-md);">
          <div>
      ...
      giftcardsContainer.innerHTML = [...individualCards, ...mixtosCards, customCard].join('');
    }
